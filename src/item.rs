...
use crate::helpers::{may_deserialize, must_deserialize, serialize};
    ...
    T: Serialize + DeserializeOwned,
{
    ...
    pub fn save(&self, store: &mut dyn Storage, data: &T) -> StdResult<()> {
        store.set(self.storage_key, &serialize(data)?);
        Ok(())
    }
    ...
    pub fn take(&self, store: &mut dyn Storage) -> StdResult<T> {
        let t = self.load(store)?;
        self.remove(store);
        Ok(t)
    }
...
mod test {
    ...
    fn update_supports_custom_errors() {
        ...
        let res = CONFIG.update(&mut store, |mut c| {
            ...
            if c.max_tokens > 10 {
                serialize(&c)?; // Uses From to convert StdError to MyError
            }
            c.max_tokens += 20;
            Ok(c)
        });
